# Load necessary R packages for Shiny, maps, spatial data, plots, and dashboard UI
library(shiny)
library(leaflet)
library(sf)
library(bslib)
...
# Load shapefile for sub-counties (second-level admin units)
sub_county_shapefile <- st_read("C:\\Users\\anitt\\Downloads\\ke_subcounty\\ke_subcounty.shp")

# Define the province names manually (since they are not present in the shapefile)
province_names <- c(
  "Central", "Coast", "Eastern", "Nairobi", "North Eastern", "Nyanza", "Rift Valley", "Western")

# Assign province names to shapefile based on the FID field
province_shapefile$Province_Name <- province_names[province_shapefile$FID + 1]

# Create a data frame with screening data per province
...
county_names <- c( "Baringo", "Bomet", "Bungoma", "Busia", "Elgeyo-Marakwet", "Embu", "Garissa", "Homa Bay", "Isiolo", "Kajiado", "Kakamega", "Kericho", "Kiambu", "Kilifi", 
                   "Kirinyaga", "Kisii", "Kisumu", "Kitui", "Kwale", "Laikipia", "Lamu", "Machakos", "Makueni", "Mandera", "Marsabit", "Meru", "Migori", "Mombasa", 
...
# Create screening data for each county
...
# Merge county screening data into the county shapefile
kenya_shapefile <- left_join(kenya_shapefile, county_screening_data, by = c("ADM1_EN" = "County"))

# List of sub-county names used for merging with shapefile
subcounty_names <- c(
  "Ainabkoi Sub County", 
  "Ainamoi Sub County", 
  "Aldai Sub County",
...
# Create screening data per sub-county
...
# Merge sub-county data into the corresponding shapefile
  ...
  nav_panel("Provinces", value = "provinces_page",
            ...
            fluidRow(
              column(8,
                     ...
                     leafletOutput("provinces_map", height = "600px") # Province choropleth
              ),
              column(4,
                     h3("Screening Rates by Province"),
                     plotOutput("caterpillar_plot_province", height = "600px") # Static plot
              )
            )
  ),
  # --- County View Tab ---
...
server <- function(input, output, session) {
  ...
  output$kenya_map <- renderLeaflet({
    # Choose map style based on dark mode toggle
    map_provider <- if (input$dark_mode == "dark") {
      providers$CartoDB.DarkMatter
    } else {
      providers$CartoDB.Positron
    }
    # Get Kenya boundaries from rnaturalearth   
  ...
  # Filter county data based on selected cancer type  
  ...
  # Render county-level static plot
  ...
  # Store selected province on map click
  selected_province <- reactiveVal(NULL)
  # Reactively get screening data for provinces
  ...
  # Render province-level caterpillar plot with highlight
